...
(all s1,s2: (one Student) {
(((s1 != s2) && (p in (s1.projects)) && (p in (s2.projects)) && (p in (((Course)).projects))) => (all g1,g2: (one Grade) {
(((g1 in (s1.(((Course)).grades))) && (g2 in (s2.(((Course)).grades)))) => ((g1 = (g2.(ordering/prev[]))) || (g2 = (g1.(ordering/prev[]))) || (g1 = g2)))
}))
})
}))
}
