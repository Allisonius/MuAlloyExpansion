...
public abstract class MutationVisitorPredicate extends VoidVisitorAdapter<Object> {
	...
	public MutationVisitorPredicate(Opt opt) {
		this.opt = opt;
		this.mi = new MInfo();
		this.domain = null;
	}
	...
	public void visit(FieldExpr n, Object arg) {
		if (domain == null || domain.contains(n)) {
			...
			mutationDataList.forEach(mutationData -> process(n, mutationData, POI.class));
			//mutationDataList.forEach(mutationData -> process(n, mutationData, UOI.class));
			//mutationDataList = UOR.mutate(n, mu, opt, mi);
		}
		super.visit(n, arg);
	}
