...
import mualloy.rule.BOD;
import mualloy.rule.BOE;
import mualloy.rule.BOR;
import mualloy.rule.FER;
import mualloy.rule.IEOE;
import mualloy.rule.LOD;
import mualloy.rule.LOR;
import mualloy.rule.MOR;
import mualloy.rule.NQDI;
import mualloy.rule.NQE;
import mualloy.rule.PBD;
import mualloy.rule.POI;
import mualloy.rule.QDI;
import mualloy.rule.QOD;
import mualloy.rule.QOI;
import mualloy.rule.QOR;
import mualloy.rule.QORU;
import mualloy.rule.QTE;
import mualloy.rule.SDR;
import mualloy.rule.SER;
import mualloy.rule.SSR;
import mualloy.rule.UOD;
import mualloy.rule.UOI;
import mualloy.rule.UOR;
import mualloy.rule.VOD;
import mualloy.rule.VOI;
...
public abstract class MutationVisitorNewOps extends VoidVisitorAdapter<Object> {
	...
	protected ArrayList<SigExpr> vistedSigExpr;

	public MutationVisitorNewOps(Opt opt) {
		this.opt = opt;
		this.mi = new MInfo();
		this.domain = null;
		this.vistedSigExpr = new ArrayList<SigExpr>();
	}
	...
	public void visit(SigDecl n, Object arg) {
		...
		if (domain == null || domain.contains(n)) {
			...
			mutationDataList = VOI.mutate(n, mu, opt, mi);
			mutationDataList.forEach(mutationData -> process(n, mutationData, VOI.class));
			mutationDataList = VOD.mutate(n, mu, opt, mi);
			mutationDataList.forEach(mutationData -> process(n, mutationData, VOD.class));
		}
		super.visit(n, arg);
	...
	public void visit(FieldDecl n, Object arg) {
		...
		if (domain == null || domain.contains(n)) {
			List<MutationData> mutationDataList = VOI.mutate(n, mu, opt, mi);
			mutationDataList.forEach(mutationData -> process(n, mutationData, VOI.class));
			mutationDataList = VOD.mutate(n, mu, opt, mi);
			mutationDataList.forEach(mutationData -> process(n, mutationData, VOD.class));
		}
	...
	public void visit(SigExpr n, Object arg) {
		if (domain == null || domain.contains(n)) {
			List<MutationData> mutationDataList = POI.mutate(n, mu, opt, mi);
			mutationDataList.forEach(mutationData -> process(n, mutationData, POI.class));
			mutationDataList = SER.mutate(n, mu, opt, mi);
			mutationDataList.forEach(mutationData -> process(n, mutationData, SER.class));
			if(!vistedSigExpr.contains(n)) {
				mutationDataList = QOI.mutate(n, mu, opt, mi);
				mutationDataList.forEach(mutationData -> process(n, mutationData, QOI.class));
				vistedSigExpr.add(n);
			}
	...
	public void visit(FieldExpr n, Object arg) {
		if (domain == null || domain.contains(n)) {
			...
			mutationDataList = POI.mutate(n, mu, opt, mi);
			mutationDataList.forEach(mutationData -> process(n, mutationData, POI.class));
			mutationDataList = FER.mutate(n, mu, opt, mi);
			mutationDataList.forEach(mutationData -> process(n, mutationData, FER.class));
		}
		super.visit(n, arg);
	}
	...
	public void visit(UnaryFormula n, Object arg) {
		if (domain == null || domain.contains(n)) {
			...
			mutationDataList = UOI.mutate(n, mu, opt, mi);
			mutationDataList.forEach(mutationData -> process(n, mutationData, UOI.class));
			mutationDataList = QTE.mutate(n, mu, opt, mi);
			mutationDataList.forEach(mutationData -> process(n, mutationData, QTE.class));
			MutationData mutationData = UOD.mutate(n, mu, opt, mi);
			if (mutationData != null) {
				process(n, mutationData, UOD.class);
			}
	...
	public void visit(BinaryExpr n, Object arg) {
		if (domain == null || domain.contains(n)) {
			...
			mutationDataList = UOI.mutate(n, mu, opt, mi);
			mutationDataList.forEach(mutationData -> process(n, mutationData, UOI.class));
			mutationDataList = BOD.mutate(n, mu, opt, mi);
			if(mutationDataList != null)
				mutationDataList.forEach(mutationData -> process(n, mutationData, BOD.class));
	...
	public void visit(BinaryFormula n, Object arg) {
		if (domain == null || domain.contains(n)) {		
			...
			mutationDataList = UOI.mutate(n, mu, opt, mi);
			mutationDataList.forEach(mutationData -> process(n, mutationData, UOI.class));
			mutationDataList = SSR.mutate(n, mu, opt, mi);
			mutationDataList.forEach(mutationData -> process(n, mutationData, SSR.class));
			mutationDataList = BOD.mutate(n, mu, opt, mi);
			if(mutationDataList != null)
				mutationDataList.forEach(mutationData -> process(n, mutationData, BOD.class));
			MutationData mutationData = BOE.mutate(n, mu, opt, mi);
	...
	public void visit(ListFormula n, Object arg) {
		if (domain == null || domain.contains(n)) {
			...
			mutationDataList = UOI.mutate(n, mu, opt, mi);
			mutationDataList.forEach(mutationData -> process(n, mutationData, UOI.class));
			mutationDataList = SDR.mutate(n, mu, opt, mi);
			mutationDataList.forEach(mutationData -> process(n, mutationData, SDR.class));
		}
		super.visit(n, arg);
	}
	...
	public void visit(QtFormula n, Object arg) {
		if (domain == null || domain.contains(n)) {
			...
			mutationDataList = UOI.mutate(n, mu, opt, mi);
			mutationDataList.forEach(mutationData -> process(n, mutationData, UOI.class));
			mutationDataList = NQDI.mutate(n, mu, opt, mi);
			mutationDataList.forEach(mutationData -> process(n, mutationData, NQDI.class));
			mutationDataList = NQE.mutate(n, mu, opt, mi);
			mutationDataList.forEach(mutationData -> process(n, mutationData, NQE.class));
			mutationDataList = QDI.mutate(n, mu, opt, mi);
			mutationDataList.forEach(mutationData -> process(n, mutationData, QDI.class));
			mutationDataList = QTE.mutate(n, mu, opt, mi);
			mutationDataList.forEach(mutationData -> process(n, mutationData, QTE.class));
			mutationDataList = QOD.mutate(n, mu, opt, mi);
			mutationDataList.forEach(mutationData -> process(n, mutationData, QOD.class));
		}
		super.visit(n, arg);
	}
	...
	public void visit(Predicate n, Object arg) {
		MType originalType = mi.getType();
		Node originalNode = mi.getNode();
		if (n.getName().contains(DOLLAR)
				|| IGNORE_NAMES.stream().anyMatch(ignoredName -> n.getName().startsWith(ignoredName))) {
			mi.setType(MType.IGNORE);
		} else {
