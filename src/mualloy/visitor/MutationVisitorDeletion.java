...
public abstract class MutationVisitorDeletion extends VoidVisitorAdapter<Object> {
	...
	public MutationVisitorDeletion(Opt opt) {
		this.opt = opt;
		this.mi = new MInfo();
		this.domain = null;
	}
	...
	public void visit(SigDecl n, Object arg) {
		...
		if (domain == null || domain.contains(n)) {
			...
			//List<MutationData> mutationDataList = VOD.mutate(n, mu, opt, mi);
	...
	public void visit(BinaryExpr n, Object arg) {
		if (domain == null || domain.contains(n)) {
			...
			mutationDataList = UOI.mutate(n, mu, opt, mi);
			mutationDataList.forEach(mutationData -> process(n, mutationData, UOI.class));*/
			List<MutationData> mutationDataList = BOD.mutate(n, mu, opt, mi);
	...
	public void visit(ListFormula n, Object arg) {
		if (domain == null || domain.contains(n)) {
			...
			List<MutationData> mutationDataList = LOD.mutate(n, mu, opt, mi);
	...
	public void visit(QtFormula n, Object arg) {
		if (domain == null || domain.contains(n)) {
			...
			mutationDataList = UOI.mutate(n, mu, opt, mi);
			mutationDataList.forEach(mutationData -> process(n, mutationData, UOI.class));
			MutationData mutationData = QORU.mutate(n, mu, opt, mi);
			if(mutationData != null) {
				process(n, mutationData, QORU.class);
			}*/
		}
		super.visit(n, arg);
	}
