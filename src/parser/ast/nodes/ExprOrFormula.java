    ...
    if (expr instanceof ExprUnary) {
      ...
      switch (binaryExpr.op) {
        ...
        case TRIGGERED:
             ...
             wrapper.setSub(new BinaryFormula(wrapper, binaryExpr));
             return wrapper;
          //return new BinaryFormula(parent, binaryExpr);
        default:
          // Wrap homogeneous binary relation with NOOP for UOI.
       //   if (AlloyUtil.isHomogeneous(binaryExpr.type())) {
        	UnaryExpr wrapper_default = new UnaryExpr(parent, binaryExpr.type(), null, UnaryExpr.UnaryOp.NOOP);
        	wrapper_default.setSub(new BinaryExpr(wrapper_default, binaryExpr));
      ...
      switch (listExpr.op) {
        case AND:
        case OR:
        	//UnaryExpr wrapper = new UnaryExpr(parent, listExpr.type(), null, UnaryExpr.UnaryOp.NOOP);
            //wrapper.setSub(new ListFormula(wrapper, listExpr));
          //  return wrapper;
      ...
      switch (qtExpr.op) {
        ...
        default:
        	UnaryExpr wrapper = new UnaryExpr(parent, qtExpr.type(), null, UnaryExpr.UnaryOp.NOOP);
            wrapper.setSub(new QtFormula(wrapper, qtExpr));
            return wrapper;
      ...
      if (ite.type().is_bool) {
    	  UnaryExpr wrapper = new UnaryExpr(parent, ite.type(), null, UnaryExpr.UnaryOp.NOOP);
          wrapper.setSub(new ITEFormula(wrapper, ite));
          return wrapper;
