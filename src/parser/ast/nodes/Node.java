...
public abstract class Node implements Visitable, Serializable {
  ...
  public List<Node> getChildren() {
	...
	public void removeChild(ExprOrFormula exprOrFormula) {
		ArrayList<Node> new_children = new ArrayList<Node>();
		for(Node child : children) {
			if(child instanceof ExprOrFormula) {
				if(child.equals(exprOrFormula)) {
	...
	public void addChild(ExprOrFormula exprOrFormula) {
		children.add(exprOrFormula);
	}
}
